...
Status: technical decisions adopted for implementation; empirical quality, publishing permission, reviewer availability, zIDE integration capabilities, and run/pilot token allocations remain unproven. This resolves the contract omissions in [the review](content-factory-v1-blindspots.md). It does not declare v1 implemented or accepted.
...
| C2 — unsafe legacy reuse | Separate video identity, source revision, and course membership. Rebuild v1 from transcripts. Old chapter/lesson outputs and completion flags are inventory/audit records, never teaching inputs or reasons to skip first-pass reprocessing. | Reconciled inventory, variant hashes, saved-result resume test, complete subtitle coverage. Local safeguards implemented; v1 input isolation and first-pass coverage still need proof. |
...
Apply the [allowed teaching sources policy](content-factory-v1-goal.md#allowed-teaching-sources--user-confirmed): transcripts, plus matching YouTube visuals only when needed for diagrams. Each manifest artifact needs an explicit role: original transcript, transcript normalization awaiting fidelity checks, legacy audit artifact, or permitted video visual. Only source-eligible artifacts may enter authoring evidence; generated lessons cannot become new source facts. Metadata is for identity/order and keyframe capture hints, not subject knowledge. The user explicitly permits using `chosen_keyframe`/`candidates_keyframes` from old chapter JSON to locate needed visuals; verify the captured pixels before treating them as evidence.
...
All v1 model work must fit the operator's zIDE subscription quota: **300,000,000 tokens**, **zero incremental cash**. Use one quota-accounting boundary and a SQLite ledger. Store token reservations and observed usage as integers, with model identity, context/input/output limits, quota period, and reservation assumptions recorded. Record measured remaining quota separately from the advertised total; other operator activity may already have consumed some of it. Numeric run and pilot allocations are still required. Direct paid API routes are outside v1.

First verify how zIDE exposes work submission, model/version selection, output export, token usage, and interrupted-job recovery. No callable API, usage export, or request-retrieval capability is assumed. If the integration is operator-mediated, describe that boundary and reconcile actual quota observations. If observability is insufficient, block automated dispatch and retain offline/fake-adapter work; do not claim the Python ledger controls subscription-wide usage. Any optional future paid route requires a new explicit cash-cap decision.
...
The selected optics course, challenge sources, and review fields are assembled in [the pilot packet](content-factory-v1-pilot.md).

Open inputs: target learner confirmation; named subject/editorial reviewer and learner access; publishing permissions for the transcripts and any needed YouTube diagram captures; verified optics source order and skipped-source disposition; zIDE integration/remaining-quota evidence; numeric run/pilot token allocations after an estimate. These are required evidence and authorization, not values to invent. Unanswered inputs block their dependent model-dispatch/release steps while offline preparation continues.
