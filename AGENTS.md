...
The inventory scans local sources with no model calls; it needs the local database and corpus and refuses a nonempty WAL. `scripts/capture_keyframes.py` recovers diagrams from chapter JSON (see [docs/factory/KEYFRAME_CAPTURE.md](docs/factory/KEYFRAME_CAPTURE.md)).
...
- **Cleaned legacy outputs are teaching sources (reversed 2026-09-07).** For each video, the cleaned counterparts in `GeminiLongContext/<playlist_id>/` (`_chapters.json`, `_v2_content.json`, `_lecture_context.json`) are approved v1 inputs alongside the raw whisper JSON; raw stays canonical for segmentation and timestamps. Derived SRT variants remain non-evidence.
- **Teaching sources:** transcripts (raw JSON plus the cleaned `GeminiLongContext/` counterparts), plus the matching YouTube lecture solely to recover a diagram that is missing or unclear.
- **Budget:** the pilot runs on the operator's zIDE quota (300M tokens, zero incremental cash). Prefer offline scripts over model calls; route heavy exploration through subagents and keep file dumps out of the main thread (see [docs/research/context-management-mcp-token-cost.md](docs/research/context-management-mcp-token-cost.md) for measured per-request overhead).
